...
/*   Updated: 2024/10/15 23:14:54 by orezek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ConnectionHandler::prepareFdSetForSelect(void)
{
	...
	// map implementation
	for (std::map<int, Client>::iterator it = ClientManager::getInstance().ClientsMap.begin(); it != ClientManager::getInstance().ClientsMap.end(); ++it)
	{
...
void ConnectionHandler::deleteClient(std::map<int, Client>::iterator &it)
{
	...
	ClientManager::getInstance().ClientsMap.erase(itToErase);
}
...
int ConnectionHandler::handleNewClients(void)
{
	...
	if (selectResponse > 0)
	{
		std::map<int, Client>::iterator it = ClientManager::getInstance().ClientsMap.begin();
		while (it != ClientManager::getInstance().ClientsMap.end())
		{
			...
			if (FD_ISSET(clientSocketFd, &readFds))
			{
				...
				else
				{
					...
					else
					{
						...
						if (it != ClientManager::getInstance().ClientsMap.end())
						{
