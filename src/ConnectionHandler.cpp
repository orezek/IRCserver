...
/*   Updated: 2024/10/15 19:36:50 by orezek           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ConnectionHandler::prepareFdSetForSelect(void)
{
	...
	// map implementation
	for (std::map<int, Client>::iterator it = Clients::getInstance().ClientsMap.begin(); it != Clients::getInstance().ClientsMap.end(); ++it)
	{
...
int ConnectionHandler::checkForNewClients(void)
{
	...
	if (FD_ISSET(this->masterSocketFd, &this->readFds))
	{
		...
		Clients::getInstance().addClient(clientSocketFd);
...
void ConnectionHandler::deleteClient(std::map<int, Client>::iterator &it)
{
	...
	Clients::getInstance().ClientsMap.erase(itToErase);
}
...
int ConnectionHandler::handleNewClients(void)
{
	...
	if (selectResponse > 0)
	{
		...
		std::map<int, Client>::iterator it = Clients::getInstance().ClientsMap.begin();
		while (it != Clients::getInstance().ClientsMap.end()) // removed serverData->clients.end()
		{
			...
			if (FD_ISSET(clientSocketFd, &readFds))
			{
				...
				else
				{
					...
					else
					{
						...
						if (it != Clients::getInstance().ClientsMap.end())
						{
